...
var config = {
	...
	timeline: {
		...
		xScale: d3.scale.linear().domain([1890,2014]).range([20, 950])
	}
}
...
function initTimeline(data) {
	...
	var xScale = config.timeline.xScale
	var yScale = d3.scale.linear().domain([1, 80]).range([height-marginH,marginW]);

	var xAxis = d3.svg.axis().scale(xScale).tickSize(1).ticks(16).tickFormat(d3.format("d"))
	var yAxis = d3.svg.axis().scale(yScale).tickSize(1).orient("right").tickFormat(d3.format("d")).tickValues([0, 20,40,60,80, 100]);
	...
	timeline.selectAll("rect")
		...
		var yScaleFlipped = d3.scale.linear().domain([1,80]).range([4, height-20]);
