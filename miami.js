...
var config = {
	...
	timeline: {
		...
		xScale: d3.scale.linear().domain([1970,2012]).range([20, 950])
	}
}
...
function initTimeline(data) {
	...
	var xScale = config.timeline.xScale
	var yScale = d3.scale.linear().domain([1, 30]).range([height-marginH,marginW]);

	var xAxis = d3.svg.axis().scale(xScale).tickSize(1).ticks(16).tickFormat(d3.format("d"))
	var yAxis = d3.svg.axis().scale(yScale).tickSize(1).orient("right").tickFormat(d3.format("d")).tickValues([1, 10, 20,30]);
	...
	timeline.selectAll("rect")
		...
		var yScaleFlipped = d3.scale.linear().domain([1, 30]).range([4, height-20]);
...
//			if(year + sliderRange == 2012 && direction == 1) {
//				direction = -1
//			}
//				
//			if(year == 1940 && direction == -1) {
//				direction = 1
//			}
			year = year + 1
		}, 100)
