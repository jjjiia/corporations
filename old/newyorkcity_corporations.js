...
function bindHandlers(){
	...
	var playing = false
	var timer;
	var timelineResetHistogram = false;
	...
	function playTimeline(sliderspan, startyear){
		...
		d3.selectAll("#play").on("click", function(){
		    d3.selectAll("#resetAll").html("Reset All")
			

			if (timelineResetHistogram){
			 	d3.select("#svgContainer3 svg").remove();
		  	    drawHistogram(histogramData());
  			  timelineResetHistogram = false
			}
