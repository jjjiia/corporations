    ...
    #for nycZip in nycZips_geo:
        ...
        for row in spamreader:
            ...
            if nycZip == row[12]:
                ...
                timelineDict[birthyear] = timelineDict.get(birthyear,0)+1
                status = row[4]
                statusDescription = row[5]
                if status == "ACTIVE":
                    companyList.append([row[0],jurisdiction,"estabilished in "+birthyear])
                else:
                    companyList.append([row[0],jurisdiction,birthyear+" - "+deathyear,statusDescription])
                if "NOT FOR PROFIT" in row[3]:
                    notForProfit +=1
                if row[4]== "ACTIVE":
                    active +=1
                    
        if len(jurisdictionDict) != 0:
            maxKey  = max(jurisdictionDict,key = lambda a: jurisdictionDict.get(a))
            maxValue = jurisdictionDict[maxKey]
            regionLength = len(list(jurisdictionDict.keys()))
            
            print "\""+ nycZip+ "\":{","\"max\":", maxValue,",\"sum\":", sum,",\"regionLength\":", regionLength,",\"active\":", active,",\"Not For Profit\": ", notForProfit,",\"timeline\":", timelineDict, ",\"values\":",jurisdictionDict,",\"regions\":", list(jurisdictionDict.keys()),",\"companies\":", companyList,"}"
