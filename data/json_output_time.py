...
years = []
for i in range(1700,2014):
    years.append(i)
#print years

with open('nyc_world_06232014.csv', 'rb') as csvfile:
    ...
    #for nycZip in nycZips_geo:
        ...
        for row in spamreader:
            ...
            if year == int(birthyear):
                ...
                zipDict[zipcode] = zipDict.get(zipcode,0)+1
                jurisdictionDict[jurisdiction] = jurisdictionDict.get(jurisdiction,0)+1
                
                status = row[4]
                statusDescription = row[5]
                if status == "ACTIVE":
                    companyList.append([row[0],zipcode,jurisdiction])
                else:
                    companyList.append([row[0],zipcode,jurisdiction,deathyear,statusDescription])
                if "NOT FOR PROFIT" in row[3]:
                    notForProfit +=1
                if row[4]== "ACTIVE":
                    active +=1
                    
        if len(zipDict) != 0:
            maxZipKey  = max(zipDict,key = lambda a: zipDict.get(a))
            maxJurisdictionKey = max(jurisdictionDict,key = lambda a: jurisdictionDict.get(a))
            maxZipValue = zipDict[maxZipKey]
            maxJurisdictionValue = jurisdictionDict[maxJurisdictionKey]
            regionLength = len(list(zipDict.keys()))
            
            print "\""+ str(year)+ "\":{","\"maxJurisdiction\":", maxJurisdictionValue,"\"maxZip\":", maxZipValue,",\"sum\":", sum,",\"active\":", active,",\"Not For Profit\": ", notForProfit,",\"jurisdiction\":", jurisdictionDict, ",\"regions\":", list(jurisdictionDict.keys()),",\"values\":",zipDict,",\"zipcodes\":", list(zipDict.keys()),",\"companies\":", companyList,"}"
