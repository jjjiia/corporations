    ...
    #for nycZip in nycZips_geo:
        ...
        for row in spamreader:
            ...
            if country == row[1]:
                ...
                if status == "ACTIVE":
                    companyList.append([row[0],zipcode,"estabilished in "+birthyear])
                else:
                    companyList.append([row[0],zipcode,birthyear+" - "+deathyear,statusDescription])
        ...
        if len(zipDict) != 0:
            maxKey  = max(zipDict,key = lambda a: zipDict.get(a))
            maxValue = zipDict[maxKey]
            regionLength = len(list(zipDict.keys()))
            
            print "\""+ country+ "\":{","\"max\":", maxValue,",\"sum\":", sum,",\"regionLength\":", regionLength,",\"active\":", active,",\"Not For Profit\": ", notForProfit,",\"timeline\":", timelineDict, ",\"values\":",zipDict,",\"zipcodes\":", list(zipDict.keys()),",\"companies\":", companyList,"}"
